...
void CosmoLevel::initialData()
{
    ...
    BoxLoops::loop(MatterConstraints<ScalarFieldWithPotential>(
                       scalar_field, m_dx, m_p.G_Newton, 0, c_Ham,
...
void CosmoLevel::postRestart()
{
    // only want to do this on the first restart and also every restart
        ...
        m_gr_amr.m_rho_mean = amr_reductions_diagnostic.sum(c_rho_scaled) / phys_vol;
        m_gr_amr.m_S_mean = amr_reductions_diagnostic.sum(c_S_scaled) / phys_vol;

        // // Set rho_mean
        // m_gr_amr.set_rho_mean(amr_reductions_diagnostic.sum(c_rho_scaled) /
        //                          phys_vol);
        pout() << "Set rho_mean = " << m_gr_amr.m_rho_mean
               << " at t = " << m_time << " on initial data at level "
               << m_level << endl;
        // m_gr_amr.set_S_mean(amr_reductions_diagnostic.sum(c_S_scaled) /
        //                        phys_vol);
        // Set K_mean
        // m_gr_amr.set_K_mean(K_total / phys_vol);
        pout() << "Calculated K mean as " << m_gr_amr.m_K_mean
...
void CosmoLevel::prePlotLevel()
{
    ...
    BoxLoops::loop(
        MatterConstraints<ScalarFieldWithPotential>(
            scalar_field, m_dx, m_p.G_Newton, 0, c_Ham, Interval(c_Mom, c_Mom)),
        m_state_new, m_state_diagnostics, EXCLUDE_GHOST_CELLS);
    CosmoDiagnostics<ScalarFieldWithPotential> cosmo_diagnostics(
        scalar_field, m_dx, m_p.G_Newton);
    BoxLoops::loop(cosmo_diagnostics, m_state_new, m_state_diagnostics,
                                 ...
                                 const double a_time)
{
    ...
    // CosmoMatterCCZ4RHS<ScalarFieldWithPotential, FourthOrderDerivatives>
        ...
        my_ccz4_matter(scalar_field, m_p.ccz4_params, m_dx, m_p.sigma,
                        m_gr_amr.m_K_mean, m_gr_amr.m_rho_mean,
                        m_gr_amr.m_S_mean, m_p.formulation, m_p.G_Newton);
